...
from app.adapters.stone.websocket import CurlingWebSocketTransport
...
def _json_response(data: dict, status_code: int = 200) -> httpx.Response:
    ...
    return httpx.Response(status_code, json=data, request=httpx.Request("POST", "http://curling.local/sys/getToken"))


def test_token_success_parses_result_token() -> None:
    """成功登录时应解析 result.token。"""

    client = FakeHttpClient(_json_response({"success": True, "code": 0, "result": {"token": "token-001"}}))
    ...
    assert client.requests[0]["url"] == "http://curling.local/sys/getToken"
    assert client.requests[0]["json"] == {"username": "user", "password": "password", "userId": "uid"}
...
def test_parse_type_1_type_2_and_type_12() -> None:
    ...
    assert isinstance(parse_raw_curling_message('{"type":12,"heartbeat":true}'), HeartbeatRawMessage)
...
def test_websocket_reconnect_uses_configured_delay() -> None:
    ...
    async def run_case() -> None:
        ...
        assert isinstance(message, HeartbeatRawMessage)
        assert sleeps == [4.5]
...
def test_websocket_receive_bytes_message() -> None:
    ...
    async def run_case() -> None:
        ...
        transport = CurlingWebSocketTransport("ws://curling.local/ws", FakeTokenClient())
        transport._websocket = websocket
        assert isinstance(await transport.receive(), HeartbeatRawMessage)
...
def test_settings_read_curling_websocket_env(monkeypatch) -> None:
    ...
    monkeypatch.setenv("CURLING_API_BASE_URL", "http://curling.local")
    monkeypatch.setenv("CURLING_WS_URL", "ws://curling.local/ws")
    ...
    assert settings.api_base_url == "http://curling.local"
    assert settings.ws_url == "ws://curling.local/ws"
